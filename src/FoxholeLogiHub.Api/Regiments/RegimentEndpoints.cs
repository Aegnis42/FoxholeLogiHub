...
public static class RegimentEndpoints
{
    ...
    public static void MapRegimentEndpoints(this WebApplication app)
    {
        ...
        app.MapPost("/api/regiments", async (CreateRegimentRequest req, ClaimsPrincipal p, AppDbContext db, ConnectionTracker tracker) =>
        {
            ...
            var officier = new RegimentRole { RegimentId = reg.Id, Name = "Officier", Permissions = (int)(RegimentPermission.ManageMembers | RegimentPermission.Invite | RegimentPermission.ManageAlliances | RegimentPermission.ResupplyRegiment | RegimentPermission.ResupplyAlliance | RegimentPermission.ResupplyManage) };
            var membre = new RegimentRole { RegimentId = reg.Id, Name = "Membre", Permissions = (int)RegimentPermission.ResupplyRegiment, IsDefault = true };
